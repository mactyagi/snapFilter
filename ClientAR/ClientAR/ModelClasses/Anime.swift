...
//  ClientAR
//
//  Created by manukant tyagi on 05/12/22.
//
...
class Anime: NSObject, VirtualContentController {
    ...
    func renderer(_ renderer: SCNSceneRenderer, nodeFor anchor: ARAnchor) -> SCNNode? {
        ...
        let faceOverlayContent = SCNReferenceNode(named: "anime")
        
        let material = SCNMaterial()
        material.diffuse.contents = UIImage(named: "anime_BaseColor")

        faceOverlayContent.childNode(withName: "anime", recursively: true)?.geometry?.materials = [material]
