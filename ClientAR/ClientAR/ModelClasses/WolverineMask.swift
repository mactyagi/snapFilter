...
#error("ARKit is not supported in iOS Simulator. Connect a physical iOS device and select it as your Xcode run destination, or select Generic iOS Device as a build-only destination.")
        ...
        print(occlusionNode.position)
        print("position")
        let faceOverlayContent = SCNReferenceNode(named: "wolverineMask")
//        faceOverlayContent.position = SCNVector3(x: occlusionNode.position.x, y: occlusionNode.position.y, z: occlusionNode.position.z)
        
        // Assign a random color to the text.
        let material = SCNMaterial()
        material.diffuse.contents = UIImage(named: "wolverine mask_BaseColor")
//        faceOverlayContent.childNode(withName: "text", recursively: true)?.geometry?.materials = [material]
        
        faceOverlayContent.childNode(withName: "wolverine", recursively: true)?.geometry?.materials = [material]
        contentNode = SCNNode()
